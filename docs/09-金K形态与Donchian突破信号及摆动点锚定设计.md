...
| 规则 | 旧锚 | 新锚 |
|---|---|---|
| 方向铁律 | 入场贴近对应角色关键位线 ±0.3% ±0.25×ATR | 入场贴近**摆动低点/高点或最近5根影线极值** ×(1±0.3%) ±0.25×ATR（5根极值恒为合格锚，补摆动点确认滞后的洞） |
| 止损锚定 | 入场侧最近关键位线 vs 5根极值取更远者，+0.3% 缓冲，<0.3×ATR 弃用 | **入场价下方最近摆动低点（多）/上方最近摆动高点（空）** vs 5根极值取更远者，其余不变 |
| 止盈锚定 | 前方第一/下一档关键位线；≤1×ATR 近线不作目标；越线拉回 0.2%；无位回退固定 RR | **recent_swings(n=50) 前方摆动点**（多=高点升序、空=低点降序）；近位过滤/拉回/固定 RR 兜底全保留；新增近距摆动点合并（`_CAND_MERGE_PCT=0.005`，防 TP1/TP2 贴脸） |
...
| 铁律实际覆盖变严 | 5 根极值恒为合格锚，远离结构的入场会被打回（旧版"无关键位放行"的口子没了） |
| 止盈锚只剩摆动点 | 创新高突破前方无摆动点 → 固定 RR 兜底（TP1=entry±1.5×止损距离、TP2=0） |
| 形态信号强度下限 0.6 | GOLDEN_12 基础强度 ≥0.5 + EMA 0.1，仍过 AI_MIN_STRENGTH=0.4 门 |
