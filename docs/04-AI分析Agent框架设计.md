...
│   → 12金K形态(最近已收盘K线) 或 收盘破N根Donchian轨+放量       │
│   → EMA21/55/144 门控(反向否决/同向加权) → signal            │
│   → position 由方向派生(看涨/向上突破=resistance,反之=support) │
└──────────────────────────┬───────────────────────────────────┘
                           ↓ ScanResult 落库
...
| 环节 | 文件 | 说明 |
|---|---|---|
| 信号生成 | `app/services/strategy/__init__.py` | 12金K形态+Donchian突破+EMA门控 统一过滤（docs/09） |
...
2. 落库的每条建议**结构合法、数值自洽**（~~方向-价格一致性~~、盈亏比程序复算通过——2026-09-15 起程序校验仅剩 RR 复算，方向-价格一致性由开仓闸门兜底）
...
- **P2 数据按需供给，工具化查询**：不再整段塞 30 根 CSV，AI 通过工具按需取（当前价、摆动结构、更早历史、资金费率…），省 token 且减少幻觉
...
- **指标预算**（一次算好供全程使用）：EMA21/55/144 状态（复用 `analyze_ema`）、ATR(14) 及其占价比、量能 20 根分位
- **布尔事实**（供技能 `use_when` 程序化判定，避免模型自行估算）：`pin_bar`（当前K线影线 > 2×实体）、`funding_extreme`（|funding| > 0.1%）、`narrow_range`（区间高度 < 1.5×ATR）等
- **信号指纹**：`sha1(symbol + signal_type + position + pattern + 收盘价按0.5%对数分桶)`（docs/09 移除关键位分量）。指纹相同 → 视为同一信号（解决 E4 的缓存键）

### Stage 1 — 规则闸门（纯程序，0 token）

进入 AI 前的最后过滤，短路掉不值得花 token 的信号：

- ~~`strength < 0.4` → 直接 skip~~（**2026-09-15 移除**：程序 skip 闸门全部退役，选中的币必须经真实 AI 分析）
- ~~24h 内同方向重复信号且 AI 已有结论 → 沿用旧结论~~（**治理前移到扫描侧**：同币同信号窗口内不落库，不重复进 AI）
...
- ~~极端行情熔断：当前 K 线振幅 > 5×ATR → 跳过~~（**2026-09-15 移除**，插针与否交 AI 判断）

### Stage 2 — Analyst 解读（轻量模型，快/便宜）

> 单 Agent 形态下此阶段被 Trader Agent 吸收（§5.1），保留作为无框架形态与降级路径。

输入：结构化事实包（信号摘要、摆动结构、EMA 状态、ATR、量能分位——**不含原始 K 线**）。
输出：四要素打分 JSON：

```json
{
  "trend":    {"score": 0-10, "note": "EMA多头排列, 斜率向上"},
  "location": {"score": 0-10, "note": "回踩摆动低点支撑位"},
...
| `get_swing_points(n)` | 最近 n 个摆动点（价格+时间） | 精确锚定结构位（docs/09 唯一结构锚源） |
...
> **【2026-09-15 拍板：校验链收缩】** 仅保留 **RR 铁律（复算 ≥1.5）** + **双评委辩论复核（默认开）** 两条。方向-价格一致性、入场偏离 ≤2%、止损锚定/宽度、强平前置、止盈锚定全部移除——锚定方法论降级为 AI 提示词纪律，程序不再打回/改挂；开仓闸门（docs/06）兜底方向-价格复核、偏离 ≤1%、RR 按现价复算、强平闸门。以下为收缩后的现行清单：

确定性校验清单（2026-09-15 收缩版）：

- Schema 合法（Pydantic）
- suggest 基本项：direction / trade_type 必填，entry/sl/tp1 为正数价格
...
- ~~方向-价格一致性~~ / ~~止损合理性（0.3~3×ATR + 影线极值锚定 + 强平上限前置）~~ / ~~止盈锚定（最近优先 + 固定 RR 兜底）~~——2026-09-15 移除，历史规则见 git 历史（锚定方法论保留在 ai_analyzer / ai_agent 提示词中）

不合格 → 把**具体违规项**作为反馈消息追加，重试 Trader（≤2 次）；仍不合格 → 强制 skip（skip_reason="风控校验未通过: ..."）。**双评委辩论复核**：对 suggest 决策多空辩论，裁判只能 keep / veto（无权改价格/方向）——**2026-09-15 起默认开启**（system_config.dual_judge_enabled），实现参考 §7.3 TradingAgents 的辩论机制移植。
...
# 工作纪律（必须遵守）
1. 提交决策前必须先核对摆动结构位：所有报价必须锚定真实结构位
   （entry/stop/tp 的 level_ref 必须解析到摆动点），禁止凭空报价格。
...
4. 证据不足、盈亏比算不过来、或大盘环境恶劣 → 果断 skip。
   skip 是合格产出，不是失败。
5. 预算：最多 8 步工具调用。建议顺序：结构位（摆动点） → 大盘/指标 →
...
| `get_swing_points(n)` | n≤20 | 最近 n 个摆动点（价格+时间索引） | 自研 | **必查**，决策锚点与结构位来源（docs/09） |
...
把交卷设计成 `submit_decision` 工具调用，校验在工具内部执行（**2026-09-15 收缩：仅 RR 铁律 + 仓位复算，其余移除**，锚定方法论保留在提示词）：

- Schema 合法（Pydantic）
- 锚点存在性（所有 `*_level_ref` 必须解析到摆动结构位）——Agent 管线的锚点换算前置检查，非校验规则
- 盈亏比复算 ≥ 1.5（对齐交易系统铁律；AI 声称值不信，用复算值落库）
- 仓位公式化（AI 不自报）
- ~~方向-价格一致性~~ / ~~止损距离 [0.3, 3]×ATR~~ / ~~止盈锚定~~——2026-09-15 移除
...
@trade_agent.tool
def get_swing_points(ctx: RunContext[AnalysisDeps], n: int = 20) -> dict:
    """摆动结构位（决策锚点，必查）"""
    return ctx.deps["signal"]["recent_swings"]
...
> **2026-09-13 更新：技能全管线生效**。P0 单次调用管线（每小时批量分析主力链路）没有工具循环、无法
> `load_skill`，此前技能仅 Agent 管线可用。现改为：`build_facts` 程序判定 use_when 命中后，
> 命中技能正文**直接注入** P0 管线系统提示词（单条 4000 / 总量 6000 字符截断，`render_matched`）；
> Agent 管线维持索引 + 按需加载。两管线共用 `skill_library.build_facts` 同一事实源。
...
---
name: range-trading
description: 震荡区间信号的专用打法：只在区间边缘顺结构位反向做，止盈看中轨
...
- **锚点模式限制表达力**：AI 只能选结构位（摆动点）做锚点，极端行情想挂"突破追多"价时表达受限 → 保留 `entry_offset_pct` 弹性 + 极端情况允许 `level_ref="market"`（市价锚）
