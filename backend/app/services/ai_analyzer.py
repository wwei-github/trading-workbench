...
- take_profit_1: 第一档止盈价，必须锚定前方结构位（做多=前一个高点/压力位，做空=前一个低点/支撑位）并留余地，盈亏比≥1.5。skip 时为 0。
- take_profit_2: 第二档止盈价，锚定更前一档同类结构位且必须比止盈一更远，盈亏比≥3。skip 时为 0。
...
1. 123法则（趋势反转）：下跌破前低(1) → 反弹不创新低(2) → 再破反弹前高点时入场(3，决定性确认)；止损放 123 转折点外侧。确认稳但滞后，入场偏晚。
2. N字结构（趋势延续）：回调不破前低 → 突破前高 → 回踩不破前低入场；止损放 N 字中间低点，破即结构失效。
3. 2B法则（假突破反手）：创新高/新低后迅速收回前高下/前低上 → 反向入场；止损放假突破极值外侧小缓冲。盈亏比最佳但胜率偏低，须严守止损。
...
- 止盈一：必须是入场方向前方的结构位——做多锚定前一个高点或压力类关键位，做空锚定前一个低点或支撑类关键位，
  并留一点余地（做多比该位低 0.2%~0.5% 提前止盈，做空相反）；若到最近结构位的盈亏比不足 1.5，
  取下一档更远的结构位作止盈一。盈亏比 ≥1.5 是开单硬性要求。
- 止盈二：取更前一档的同类型结构位（再前一个高点/低点或关键位），同样留余地；止盈二必须比止盈一更远。
