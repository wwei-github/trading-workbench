...
- 优先：做多锚定入场价下方最近支撑位的区域下轨（zone_low）外 0.2%（建议 0.2%~0.5%），做空锚定入场价上方最近压力位的区域上轨（zone_high）外同幅度；锚须落在最近 10 根K线影线范围内（即"10根之内有关键位"）。
- 兜底：入场价与最近 10 根影线极值之间没有任何同类关键位时，用极值锚——做多严格低于最近 10 根最低点（影线极值即最低价，非收盘价）至少 0.2%，做空严格高于最高点至少 0.2%。
