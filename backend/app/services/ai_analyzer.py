...
止损止盈缓冲纪律（多空方向不同，必须遵守）：
- 做多：止损位必须严格低于最近 10 根K线的最低点（不能正好用最低点，酌情再低 0.2%~0.5% 防插针扫损）；
  止盈位要比目标压力位酌情低一点（提前 0.2%~0.5% 止盈），防价格到不了目标位就回落。
- 做空相反：止损位必须严格高于最近 10 根K线的最高点（不能正好用最高点，酌情再高 0.2%~0.5%）；
  止盈位比目标支撑位酌情高一点。
- 若止损越过最近10根极值后超过 3% 止损红线或盈亏比不达标，优先调整入场价，仍不行则 skip。
