...
- entry_price: 建议入场价。必须以"当前价格"（分析时刻最新价）为基准：顺势追势时入场贴近现价；
  ...
  skip 时为 0。
- 方向铁律（程序强校验，违规直接打回）：只在支撑位做多，只在压力位做空——入场价必须落在
  信号方向对应角色（多=support、空=resistance）的关键位区域内。唯一例外：信号类型为 breakout
  （放量突破，收盘越过整个关键位区域+量能≥1.2×均量+EMA同向）时顺势追突破，入场贴近现价。
- stop_loss: 止损价，做多时低于入场价，做空时高于入场价，基于结构位。skip 时为 0。
- take_profit_1: 第一档止盈价，做多=上方压力位区域下轨（zone_low）下方留 0.2%~0.5% 余地，
  做空=下方支撑位区域上轨（zone_high）上方留 0.2%~0.5%，盈亏比≥1.5。skip 时为 0。
- take_profit_2: 第二档止盈价，锚定更前方一档关键位的区域近轨（做多=更高一档压力位的下轨下方，
  做空=更低一档支撑位的上轨上方，同样留余地）且必须比止盈一更远。skip 时为 0。
...
- 优先：做多锚定入场价下方最近支撑位的区域下轨（zone_low）外 0.3%（建议 0.3%~0.5%），做空锚定入场价上方最近压力位的区域上轨（zone_high）外同幅度；锚须落在最近 10 根K线影线范围内（即"10根之内有关键位"）。
- 两锚取更远者：入场贴着关键位区域是区间边缘类开单的常态，此时区域近轨在 10 根影线范围内，只锚关键位会把止损放进近期波动区间内部、极易被扫损。止损必须同时在关键位外与影线极值外——做多取 zone_low 与最近 10 根最低点中更低者外 0.3%，做空取 zone_high 与最近 10 根最高点中更高者外 0.3%。
...
- 关键位是带上下轨的区域（列表中"区域低~高"，重叠/近邻的同角色区域已预先合并为一档）。
  止盈一必须挂在入场方向前方第一个结构位的区域近轨外侧：
  做多 = 上方压力位的下轨（zone_low）下方 0.2%~0.5%，不得挂在压力位区域内；
  做空 = 下方支撑位的上轨（zone_high）上方 0.2%~0.5%，不得挂在支撑位区域内。
  价格常在区域边缘反弹，深入区域才触发的止盈大概率落空。
- 禁止把止盈一挂到更远结构位凑盈亏比：到第一结构位的盈亏比不足 1.5 说明这笔交易
  空间不足（第一压力/支撑都够不着 1.5R），应直接 skip，不要跳档。程序会把偏离的
  止盈一强制改挂回第一结构位近轨外侧，再按盈亏比铁律打回。
- 止盈二：挂在止盈一锚定档的下一档结构位近轨外侧（做多=更高一档压力位的下轨下方，
  做空=更低一档支撑位的上轨上方），同样留余地；止盈二必须比止盈一更远。
  近期摆动高点/低点不算独立锚定档——仅当前方没有任何关键位区域时才作回退锚
  （锚定价位本身，留同样余地）。
- 若入场方向前方没有任何结构位可锚定（如创新高突破）：止盈一 = 入场价 ± 1.5×止损距离（盈亏比恰达标），
  止盈二设 0（仅设一档）；程序也会自动按此回退。

关键位可靠性：关键位列表中触及次数越多、时间加权权重越高越可靠；时间加权已含形态确认加成
（触及点出现方向匹配的 12 金K 会放大权重，"形态确认N次"标记）。锚定止盈、评估支撑压力强度时
优先选触及多/有权重/形态确认多的位。
