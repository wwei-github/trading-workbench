...
- order_type: 委托方式（suggest 时按形态性格二选一，程序按你的选择直接执行）：
  "market"=市价立即成交；"limit"=限价委托，以 entry_price 挂单等待回踩/反抽成交
  （12 小时未成交自动撤销，成交价只会比委托价更优）。
  选择依据：突破追入/动量延续/区间边缘当场触发——选 market（等回踩会踏空）；
  回踩企稳、反抽确认、等二次测试类计划（analysis 中写了"等回踩/等反抽"）——选 limit，
  此时 entry_price 即挂单价，通常偏离现价（挂低价等回踩买入/挂高价等反抽卖出）。
  skip 时为空字符串。
- entry_price: 建议入场价。order_type=market 时必须贴近"当前价格"（分析时刻最新价，偏离现价>2%会被程序打回）；
  order_type=limit 时为挂单价——给回踩锚定价（摆动低/高点）等被动成交；若现价已显著离开
  摆动结构位且无合理入场计划，应 skip。skip 时为 0。
- 方向铁律（程序强校验，违规直接打回）：只在支撑结构做多，只在压力结构做空——入场价必须贴近
  信号方向的摆动低点/最近N根最低点（做多）或摆动高点/最近N根最高点（做空）（位 ±0.3% 容差带内）。
  唯一例外：信号类型为 breakout（放量突破，收盘破 Donchian 通道+量能≥1.2×均量+EMA同向）时
  顺势追突破，入场贴近现价。
...
止损纪律（结构位优先，影线极值兜底，两锚取更远者；程序按此规则定锚，AI 给值仅参考）：
...
止盈锚定（多空方向不同，必须遵守；程序强制最近优先，偏离会直接改挂）：
...
- 禁止把止盈一挂到更远结构位凑盈亏比：到第一结构位的盈亏比不足 1.5 说明这笔交易
  空间不足（第一结构位都够不着 1.5R），应直接 skip，不要跳档。程序会把偏离的
  止盈一强制改挂回第一结构位外侧，再按盈亏比铁律打回。
