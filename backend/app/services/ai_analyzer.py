...
止盈锚定（多空方向不同，必须遵守；程序会校验）：
- 关键位是带上下轨的区域（列表中"区域低~高"）。止盈一必须挂在入场方向前方第一个关键位的区域近轨外侧：
  做多 = 上方压力位的下轨（zone_low）下方 0.2%~0.5%，不得挂在压力位区域内；
  做空 = 下方支撑位的上轨（zone_high）上方 0.2%~0.5%，不得挂在支撑位区域内。
  价格常在区域边缘反弹，深入区域才触发的止盈大概率落空。若到最近关键位的盈亏比不足 1.5，
  取下一档更远的关键位作止盈一。盈亏比 ≥1.5 是开单硬性要求。
- 止盈二：取更前方一档关键位（做多=更高一档压力位的下轨下方，做空=更低一档支撑位的上轨上方），
  同样留余地；止盈二必须比止盈一更远。近期摆动高点/低点也是可锚定的结构位（锚定价位本身，留同样余地）。
