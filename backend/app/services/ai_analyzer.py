...
- entry_price: 建议入场价。必须以"当前价格"（分析时刻最新价）为基准：顺势追势时入场贴近现价；
  ...
  skip 时为 0。
- 方向铁律（程序强校验，违规直接打回）：只在支撑位做多，只在压力位做空——入场价必须贴近
  信号方向对应角色（多=support、空=resistance）的关键位价格线（线 ±0.3% 容差带内）。唯一例外：
  信号类型为 breakout（放量突破，收盘越过关键位容差带+量能≥1.2×均量+EMA同向）时顺势追突破，
  入场贴近现价。
- stop_loss: 止损价，做多时低于入场价，做空时高于入场价，基于结构位。skip 时为 0。
- take_profit_1: 第一档止盈价，做多=上方压力位价格线下方留 0.2%~0.5% 余地，
  做空=下方支撑位价格线上方留 0.2%~0.5%，盈亏比≥1.5。距现价特别近（≤1×ATR）的线
  是当前争夺位、不算止盈目标，止盈一从更远的第一条线起算。skip 时为 0。
- take_profit_2: 第二档止盈价，锚定更前方一档关键位的价格线（做多=更高一档压力线下方，
  做空=更低一档支撑线上方，同样留余地）且必须比止盈一更远。skip 时为 0。
...
止损纪律（2026-09-13 更新：关键位优先，影线极值兜底；2026-09-14 补强：两锚取更远者；程序按此规则定锚，AI 给值仅参考）：
- 优先：做多锚定入场价下方最近支撑位价格线外 0.3%（建议 0.3%~0.5%），做空锚定入场价上方最近压力位价格线外同幅度；锚须落在最近 10 根K线影线范围内（即"10根之内有关键位"）。
- 两锚取更远者：入场贴着关键位是区间边缘类开单的常态，此时线位在 10 根影线范围内，只锚关键位会把止损放进近期波动区间内部、极易被扫损。止损必须同时在关键位外与影线极值外——做多取支撑线与最近 10 根最低点中更低者外 0.3%，做空取压力线与最近 10 根最高点中更高者外 0.3%。
- 兜底：入场价与最近 10 根影线极值之间没有任何同类关键位时，用极值锚——做多严格低于最近 10 根最低点（影线极值即最低价，非收盘价）至少 0.3%，做空严格高于最高点至少 0.3%。
- 止损宽度不设固定百分比上限；若盈亏比不达标，优先调整入场价，仍不行则 skip。

止盈锚定（多空方向不同，必须遵守；2026-09-14 起程序强制最近优先，偏离会直接改挂）：
- 关键位是单价格线（列表中"支撑/压力 + 价格"，角色由来源固定：摆动低点=支撑、摆动高点=压力）。
  **近线不算止盈目标（2026-09-14）**：距现价特别近（≤1×ATR）的线是价格正在测试/突破的
  "争夺位"——价格上方的即压力位、下方的即支撑位，它回答的是"价格是否得到支撑/压力/突破"，
  不是盈利空间；止盈一必须挂在更远的第一条结构位价格线外侧：
  做多 = 上方压力位价格线下方 0.2%~0.5%，不得挂在压力线上方或贴死线位；
  做空 = 下方支撑位价格线上方 0.2%~0.5%，不得挂在支撑线下方或贴死线位。
  价格常在线附近反弹，深入越线才触发的止盈大概率落空。
- 禁止把止盈一挂到更远结构位凑盈亏比：到第一结构位的盈亏比不足 1.5 说明这笔交易
  空间不足（第一压力/支撑都够不着 1.5R），应直接 skip，不要跳档。程序会把偏离的
  止盈一强制改挂回第一结构位线外侧，再按盈亏比铁律打回。
- 止盈二：挂在止盈一锚定档的下一档结构位线外侧（做多=更高一档压力线下方，
  做空=更低一档支撑线上方），同样留余地；止盈二必须比止盈一更远。
  近期摆动高点/低点不算独立锚定档——仅当前方没有任何关键位时才作回退锚
  （锚定价位本身，留同样余地）。
- 若入场方向前方没有任何结构位可锚定（如创新高突破）：止盈一 = 入场价 ± 1.5×止损距离（盈亏比恰达标），
  止盈二设 0（仅设一档）；程序也会自动按此回退。

关键位可靠性：关键位列表中触及次数（touches）越多、形态确认（pattern_hits，触及点出现方向
匹配的 12 金K）越多越可靠。锚定止盈、评估支撑压力强度时优先选触及多/形态确认多的位。
...
def _build_messages(
    ...
    # 只按 price/touches/pattern_hits 读取（price 恒存在）——历史行带 zone/weight 字段也兼容
    ...
    if signal.get("key_levels"):
        ...
        for lv in signal["key_levels"]:
            ...
            lines.append(f"  {kind}: {lv['price']:.6g} ({role}, {touch})")
