...
止损纪律（多空方向不同，必须遵守）：
- 做多：止损位必须严格低于最近 10 根K线的最低点（用影线极值即最低价计算，不是收盘价），且至少低 0.2%（建议 0.2%~0.5%）防插针扫损；程序强校验，缓冲不足会自动推远。
- 做空相反：止损位必须严格高于最近 10 根K线的最高点（影线极值，非收盘价），且至少高 0.2%（建议 0.2%~0.5%）。
- 止损宽度不设固定百分比上限；若越过最近10根极值后盈亏比不达标，优先调整入场价，仍不行则 skip。
