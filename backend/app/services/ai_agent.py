...
9. 盈亏比铁律 ≥{rr_min}；止损宽度由结构位决定，不设固定价格百分比上限；仓位不要自己报，
   程序按风险预算计算（触发止损时账户亏损不超过 3% 资金，止损越远仓位越小）。
10. 止损止盈缓冲纪律（多空方向不同）：做多——stop 必须严格低于最近 5 根K线最低点（不能正好用最低点，
    酌情再向下留 0.2%~0.5% 缓冲防插针扫损）；做空相反，stop 必须严格高于最近 5 根K线最高点。
    tp 相对目标位留 0.2%~0.5% 提前止盈缓冲。留缓冲后仍须满足盈亏比铁律，不满足时调整 entry，仍不行则 skip。
