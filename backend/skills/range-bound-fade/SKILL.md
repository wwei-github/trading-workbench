...
**前提**：`signal_type == "range_bound"`，区间边界由近期摆动点构成（上方边界=压力位、下方边界=支撑位）。

**入场**：区间底部出现看涨金K → 做多（锚点支撑位 support）；区间顶部出现看跌金K → 做空（锚点压力位 resistance）。止损锚点放区间外侧（做多止损 = 支撑位锚点再向下偏移 0.5×ATR，offset 为负）。

**止盈**：做多 tp1 = 上方压力位（对侧边界，留 0.2%~0.5% 余地）；做空 tp1 = 下方支撑位；tp2 = 更远一档结构位（摆动点）。区间太窄导致复算盈亏比不足时直接 skip——区间交易不追盈亏比。
